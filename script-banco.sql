...
--
-- Alteracoes em relacao ao script original (todas aditivas, explicadas no README):
--   [1] UNIQUE (AlunoId, TurmaId) em dbo.Matricula  -> impede matricula duplicada no nivel do banco
--   [2] CHECK em dbo.Turma.VagasDisponiveis         -> protege o invariante 0 <= disponiveis <= total
--   [3] CHECK em dbo.Turma.Periodo                  -> dominio ja descrito no comentario da coluna
--   [4] indice em dbo.Aluno.Nome                    -> suporta o filtro/ordenacao da listagem paginada
...
-- [4] filtro "por nome" + ORDER BY Nome da listagem paginada
...
CREATE TABLE dbo.Turma (
    ...
    -- [2] a transacao de matricula mantem esse invariante; o CHECK e a ultima linha de defesa
    CONSTRAINT CK_Turma_Vagas CHECK (VagasDisponiveis >= 0 AND VagasDisponiveis <= VagasTotal),
    -- [3] dominio ja documentado no comentario da coluna
    CONSTRAINT CK_Turma_Periodo CHECK (Periodo IN ('Manha', 'Tarde', 'Noite'))
);
...
CREATE TABLE dbo.Matricula (
    ...
    -- [1] regra "um aluno nao se matricula duas vezes na mesma turma" garantida pelo banco
